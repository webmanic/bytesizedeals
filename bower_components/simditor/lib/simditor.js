...
Simditor = (function(superClass) {
  ...
  Simditor.prototype.getValuePlain = function() {
    return this.syncPlain();
  };

  Simditor.prototype.getWordCount = function() {
  ...
  Simditor.prototype.syncPlain = function() {
    var children, cloneBody, emptyP, firstP, lastP, val;
    cloneBody = this.body.clone();
    this.formatter.undecorate(cloneBody);
    this.formatter.format(cloneBody);
  ...
  function wordCount(el) {
    ...
    {
        string = paragraphs[i].innerHTML.replace(/&(nbsp|amp|quot|lt|gt);/g," ").replace(/\s\s+/g, ' ').trim();
        string = string.replace(/<(?:.|\n)*?>/gm, '');
        string = string.replace(/[`~!@#$%^&*()_|+\-=?;:'",.<>\{\}\[\]\\\/]/gi, '').replace(/\s\s+/g, ' ').trim();
        count += string.length > 0 ? string.split(' ').length : 0;
    }
    return count;
  }

  Simditor.prototype.syncWordCount= function() {
    var children, cloneBody, emptyP, firstP, lastP, val;
    cloneBody = this.body.clone();
    this.formatter.undecorate(cloneBody);
    this.formatter.format(cloneBody);
